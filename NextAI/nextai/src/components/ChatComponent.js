...
const ChatComponent = (props) => {
  const { handleResp, isLoading, setIsLoading } = props; //使用解构语法，从props中获取handleResp和isLoading属性
  const [searchValue, setSearchValue] = useState(""); //初始化searchValue为空字符串
  const [isChatModeOn, setIsChatModeOn] = useState(false); //初始化isChatModeOn为false
  const [isRecording, setIsRecording] = useState(false); //初始化isRecording为false
  const [speech, setSpeech] = useState(); //初始化speech为空
